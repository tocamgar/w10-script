...
ffmpeg -y -i %VIDEO% -i %LOGO% -filter_complex "[0:v] scale=512:-2:flags=bicubic [v1];[v1][1:v]overlay=x=(main_w-overlay_w-5):y=(main_h-overlay_h-5),drawtext=fontfile='C\:/Windows/Fonts/impact.ttf':fontcolor=white:borderw=1:fontsize=18:alpha=0.67:x=(w-tw)/2:y=5:text=%TEXTO%" -b:v 150k -b:a 56k %VIDEO:~0,-5%_288p_150k_56k.mp4"